...
export function InteractiveCLI() {
  ...
  useEffect(() => {
    const bootSequence = async () => {
      const bootLines = [
        ...
        { id: '4', text: 'user: Karunya_Muddana', type: 'system' as const, delay: 3900 },
    ...
    for (const line of ASCII_ART) {
      await new Promise((resolve) => {
        setTimeout(() => {
          ...
          resolve(null);
        }, 220); // Slower, more deliberate timing
      });
    }
  ...
  const typeCharByChar = async (text: string, type: OutputLine['type'] = 'output') => {
    ...
    for (const char of text) {
      currentText += char;
      await new Promise((resolve) => {
        // Variable delay: 50-60ms with slight randomization for "thinking" effect
        const delay = 50 + Math.random() * 10;
        setTimeout(() => {
          setOutputLines((prev) =>
            prev.map((line) =>
    ...
    // Pause after line completes (200ms between lines)
  ...
  const handleAIQuery = async (query: string) => {
    ...
    // Simulate neural processing delay (2.5-3.5 seconds)
    const thinkingDelay = 2500 + Math.random() * 1000;
    ...
    try {
      if (connectionStatus === 'online') {
        ...
        const response = await fetch('https://flask-1yw5.onrender.com/chat', {
          method: 'POST',
        ...
        // Display AI response with cinematic typing
      ...
      // Simulated response when backend is unavailable
  ...
  const executeCommand = async (cmd: string, silent = false) => {
    ...
    if (!silent) {
      setScreenDim(true);
      setTimeout(() => setScreenDim(false), 150);
      addLine(`${isRoot ? 'Karunya@root:/$' : 'Karunya@terminal:~$'} ${cmd}`, 'command');
    ...
    // Half-second pause before major operations
    ...
    switch (trimmedCmd) {
      ...
      case 'about':
        ...
        await typeOutLines([
          { text: '' },
          { text: 'Name: Karunya Muddana', typed: true },
          { text: 'Role: Backend & AI Developer', typed: true },
          { text: 'Location: Hyderabad, India', typed: true },
          { text: '' },
        ], 80);
        await typeOutLines([
      ...
      case 'contact':
        ...
        await typeOutLines([
          { text: '' },
          { text: 'Email: karunya.muddana@outlook.com', copyable: true },
          { text: 'GitHub: github.com/Karunya-Muddana', copyable: true },
          { text: 'LinkedIn: linkedin.com/in/karunya-muddana-32b118269', copyable: true },
      ...
      case 'exit':
        ...
        // Fade to black with afterglow
        await new Promise((resolve) => setTimeout(resolve, 800));
        
        // Keep ASCII logo with faint afterglow before full dark
      ...
      case 'run ai_agent':
        ...
        try {
          const testResponse = await fetch('https://flask-1yw5.onrender.com/chat', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
      ...
      // Easter egg: version check
      case 'karunya --version':
      case '--version':
        await typeOutLines([
          { text: '' },
          { text: 'Karunya CLI v2.0.0' },
          { text: 'Build date: 2025-11-07' },
  ...
  return (
    <div className="min-h-screen bg-[#1B1B1B] text-white font-mono relative overflow-hidden">
      {/* Matrix background with two-layer parallax */}
      <MatrixBackground />
      ...
      {/* Cinematic volumetric lighting gradient */}
      <motion.div 
      ...
      {/* Monitor glow - subtle gold rim-light on edges */}
      <div 
      ...
      {/* Reflection gradient below text (glass desk effect) */}
      <div 
      ...
      {/* Animated cinematic scanline - more subtle */}
      <motion.div
      ...
      {/* Subtle horizontal scanlines - reduced opacity */}
      <div 
      ...
      {/* Voltage dip overlay on command input */}
      <motion.div
      ...
      {/* AI Thinking Signal Wave Animation */}
      {isAiThinking && (
        <motion.div
      ...
      {/* Terminal content with cinematic text container */}
      <div 
        ref={terminalRef}
      ...
      >
        <div className="max-w-4xl mx-auto">
          {/* Output lines with phosphor decay and fade-up */}
          <AnimatePresence>
            {outputLines.map((line, index) => (
              ...
              >
                {line.text}
                {/* Phosphor decay on fresh characters */}
          ...
          {/* Command prompt with cinematic cursor */}
      ...
      {/* AI Mode Helper Message (Desktop) */}
